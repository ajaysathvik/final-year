...
ARTIFACT_DIR = BASE_DIR / "artifacts" / "xgb_balanced_train_imbalanced_test_3k"
...
INPUT_TRAIN_FILE = BASE_DIR.parent / "Data labeling" / "outputs" / "train_3k.csv"
INPUT_TEST_FILE = BASE_DIR.parent / "Data labeling" / "outputs" / "test_3k.csv"
...
REPORT_FILE = BASE_DIR / "reports" / "XGBOOST_IMBALANCED_TEST_REPORT_3K.md"
...
def plot_roc(y_true, y_score):
    ...
    plt.title("Balanced-Train / Imbalanced-Test XGBoost 3K ROC Curve")
...
def write_report(metrics: dict):
    report_text = f"""# XGBoost Imbalanced Test Report (3K)

Date: 2026-03-26

## Overview

This report summarizes an XGBoost baseline trained on a balanced training subset and evaluated on an imbalanced held-out test set from `{INPUT_TEST_FILE.name}`.
...
def main():
    train_full = pd.read_csv(INPUT_TRAIN_FILE)
    test_full = pd.read_csv(INPUT_TEST_FILE)
    
    train_full = train_full[train_full[TARGET].isin([0, 1])].copy()
    test_full = test_full[test_full[TARGET].isin([0, 1])].copy()

    train_not_fraud = train_full[train_full[TARGET] == 0].copy()
    train_fraud = train_full[train_full[TARGET] == 1].sample(
        n=99 * len(train_not_fraud), random_state=RANDOM_STATE, replace=True
    )
    train_balanced = (
    ...
    X_train = train_balanced.drop(columns=LEAKY_COLUMNS, errors="ignore")
    y_train = train_balanced[TARGET].astype(int)
    X_test = test_full.drop(columns=LEAKY_COLUMNS, errors="ignore")
