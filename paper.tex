...
\begin{itemize}
    ...
    \item \textbf{Hyperparameter Optimization:} Automatically searches for the best hyperparameters (epochs, batch size, learning rate, network dimensions, PAC) to maximize the statistical similarity between real and synthetic data.
...
Before evaluating the adversarial robustness, we systematically validated the standard CTGAN generation logic by comparing real versus synthetic data density constraints. The optimal hyperparameters used were: 600 epochs, a batch size of 16, a learning rate of 0.00038, 32 hidden dimensions, 2 generator layers, and 3 discriminator layers.
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.45\textwidth]{outputs/data_distribution_tsne.png}}
\caption{Data Distribution (t-SNE) for Original vs Synthetic Data.}
\label{fig:tsne}
\end{figure}

\begin{figure}[htbp]
\centerline{\includegraphics[width=0.45\textwidth]{outputs/data_distribution_pca.png}}
...
The run artifacts support a more detailed robustness analysis than clean versus attacked accuracy alone. In particular, we evaluated robustness using three complementary criteria: average attacked performance, stability across resamples, and worst-case attacked performance. Figures \ref{fig:attack_curves_actual} and \ref{fig:robustness_criteria_actual} summarize the two recorded experimental configurations extracted from the pipeline logs. Run 1 achieved the slightly higher bootstrap mean attacked F1 ($0.9062$ versus $0.9057$) and the lower bootstrap variance ($3.48\times10^{-5}$ versus $4.49\times10^{-5}$), indicating marginally better average robustness and stability. However, Run 2 achieved the clearly better worst-case attacked F1 ($0.8695$ versus $0.8310$), with its worst point occurring at $\epsilon=0.15$, and also the higher aggregate robustness score ($0.8819$ versus $0.8632$). This indicates that the later adversarially hardened configuration traded a negligible amount of average robustness for substantially stronger resilience under the most challenging attack settings.
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.45\textwidth]{outputs/robustness_comparison.png}}
\caption{Adversarial Robustness Under Evasion Attacks.}
\label{fig:robustness}
\end{figure}

\begin{figure}[htbp]
\centerline{\includegraphics[width=0.48\textwidth]{agent/output/plots/run_comparison_attack_curves.png}}
\caption{Recorded attacked F1 curves across the two experimental configurations.}
\label{fig:attack_curves_actual}
\end{figure}

\begin{figure}[htbp]
\centerline{\includegraphics[width=0.49\textwidth]{agent/output/plots/run_comparison_robustness_criteria.png}}
\caption{Robustness comparison using average attacked F1, variance across resamples, and worst-case attacked F1.}
\label{fig:robustness_criteria_actual}
\end{figure}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.45\textwidth]{outputs/classifier_metrics_comparison.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.45\textwidth]{outputs/aucroc_heatmap.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.45\textwidth]{outputs/ood_comparison.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.45\textwidth]{outputs/ood_accuracy_drop_heatmap.png}}
